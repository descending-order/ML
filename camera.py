...
# GitHub:   https://github.com/coneypo/D 
# 调用摄像头实时单个/多个人脸检测，并依次在摄像头窗口，实时平铺显示检测到的人脸；
...
def main():
    ...
    predictor = dlib.shape_predictor('D:\人脸识别\data\dlib\shape_predictor_68_face_landmarks.dat')

    # 创建 cv2 摄像头对象
    cap = cv2.VideoCapture("sample-videos/head-pose-face-detection-male.mp4")
...
def main():
    # 储存截图的目录
    path_screenshots = "images/screenshots/"
        ...
        if k == ord('q'):
            ...
            if len(faces) != 0:
                ...
                for face in faces:
                    ...
                    face_shape = predictor(img_rd, face)
                    face_chip = dlib.get_face_chip(img_rd, face_shape)
                    cv2.imshow("68landmarks", face_chip)
                ...
                cv2.putText(img_rd, "Faces in all: " + str(len(faces)), (20, 350), font, 0.8, (0, 0, 0), 1, cv2.LINE_AA)
                for i in range(68):
                     cv2.putText(img_rd, str(i), (face_chip.part(i).x, face_chip.part(i).y), cv2.FONT_HERSHEY_DUPLEX, 0.1, (0, 255,0 ), 1, cv2.LINE_AA)
                     cv2.circle(img_rd, (face_chip.part(i).x, face_chip.part(i).y), 1, (0, 0, 255))
                     cv2.imshow("rotated", img_rd)
                     cv2.waitKey(1)
